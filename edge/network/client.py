...
Signing semantics (v0.3.1, 2026-05-18):
  Edge signs the Poseidon payloadHash. The Noir circuit (zk/circuits/v08)
  verifies signature over payload_hash; V3 contract verifies the same
  signature over the same payloadHash. All three layers agree.

  EIP-712 typed signing was attempted in v0.3 but reverted because the
  Noir circuit cannot be updated to verify EIP-712 digest without a
  multi-day circuit redesign (Keccak inside Noir is prohibitively
  expensive). The EIP-712 infrastructure (hal/eip712.py module, V3
  domainSeparator + eip712Digest view functions) is retained for future
  signed-message features (admin operations, off-chain orders).
...
class AggregatorClient:
    ...
    def submit(self, payload: CanonicalPayload) -> SubmissionResponse:
        """Sign payloadHash (Poseidon) and submit to aggregator.

        Signing flow:
          1. Compute Poseidon payloadHash from canonical bytes
          2. Sign payloadHash with P-256 device key
          3. POST {payload, signature, public_key} to /submissions

        The aggregator computes its own payloadHash, verifies the signature,
        generates witness/proof, and submits to V3 — which verifies the same
        signature against the same payloadHash. All three layers agree.
