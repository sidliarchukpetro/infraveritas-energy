...
Signing semantics (v0.3.1, 2026-05-18):
  Edge signs the Poseidon payloadHash. Same signature is verified by:
    - aggregator pre-check (worker/index.ts)
    - Noir circuit constraint (zk/circuits/v08 verify_signature)
    - V3 contract CHECK 2 (P256VerifierAdapter)

Run AFTER aggregator started in live mode. From edge/ root with venv:
